...
Note: There are no form of validation and coersion on env variables.
If you need this features please checkout pydantic_settings.py
...
class Settings(metaclass=SingletonMeta):
    ...
    def __post_init__(self) -> None:
        ...
        self._set_if_notnone("PRIVATE_INT", os.getenv("PRIVATE_INT"))
        self._set_if_notnone("PRIVATE_STR", os.getenv("PRIVATE_STR"))
        self._set_if_notnone("PUBLIC_INT", os.getenv("PUBLIC_INT"))
        self._set_if_notnone("PUBLIC_STR", os.getenv("PUBLIC_STR"))
...
if __name__ == "__main__":
    config = Settings()
    var = config.PUBLIC_INT
    var = config.PUBLIC_STR
    var = config.PRIVATE_INT
    logger.info(config)
